...
def regular_parsing_xml(document):
    ...
    prec = ""
    post = ""
    #When each element is found, the antecedent and consequent separators are saved in strings
...
def restriction_filter(document,newIndexes,j = None):
    ...
    for h in range(1,len(newIndexes)):
        document[newIndexes[0]] = str(document[newIndexes[0]]) + "," + str(document[newIndexes[h]])
    ...
    for j in columns:
        ...
        if attributeAcc or valueAcc:
            ...
            for stringIndex in columnsKey:
                ...
                newDocument = document.map(lambda x: restriction_filter(x,newIndexes,j))
                ...
                #calculate the distance between each value and the expected mean, and calculate each accuracy value as this distance divided by the maximum allowed interval
		...
		if globalAcc:
			...
			attributeStaticAccuracy = globalAccuracies[2] / float(globalAccuracies[0])
			attributeAccuracy = globalAccuracies[1] / float(globalAccuracies[0])

                accuracyRdd = staticAccuracyRdd.map(lambda x: (x[0],x[1][1]/float(x[1][0]),x[1][2]/float(x[1][0])))
                ...
                if valueAcc:
                    save_json(accuracyRdd,["Value","AccuracyDynamic","AccuracyStatic"],resultFolder + "/accuracy_values/attribute_" + stringAttribute + "_REF_" + str(desiredColumns[j]))
                
                if attributeAcc:
                    finalAccuracyRdd = finalAccuracyRdd.union(sc.parallelize([(stringAttribute,accuracyRdd.map(lambda x: x[1]).mean(),accuracyRdd.map(lambda x: x[2]).mean(),performanceSample)]))
                
                
            if attributeAcc:
                #save file into hdfs
                save_json(finalAccuracyRdd,["Attribute","AccuracyDynamic","AccuracyStatic","Confidence"],resultFolder + "/accuracy_attributes_" + str(desiredColumns[j]))
...
def calculateSampleDeviation(line):
    #lambda (key,(sumDev,count,sumMean)): (key,((sumDev/(count-1))**0.5,sumMean/count,count))
    try:
        newLine = (line[0], ( (line[1][0]/(line[1][1]-1))**0.5, line[1][2]/float(line[1][1]), float(line[1][1]) ) )
    except Exception:
        newLine = (line[0], (0, line[1][2]/float(line[1][1]), float(line[1][1]) ) )
...
def precision(sc,sqlContext,document,columns,dataTypes,volatiliy,desiredColumns,resultFolder,dimensionColumn,columnsKey,meanAccuracyValues,devAccuracyValues,timelinessAnalyzer,completenessAnalyzer,distinctnessAnalyzer,populationAnalyzer,inputSource,associationRules,totVolume,granularityDimensions,performanceSample):
    ...
    for j in columns:
        ...
        reShift = False
        statsDoc = document.map(lambda x: (float(x[j].replace(',', '.')))).stats()
        ...
        if attributePrec or valuePrec:
            ...
            for stringIndex in columnsKey:
                ...
                keyFloatDocument = newDocument.map(lambda x: (x[0],float(x[1].replace(',', '.'))))
                        
                if reShift:
                    print("---Shifting all the values to make them greater than 1 ")
                    keyFloatDocument = keyFloatDocument.map(lambda x: (x[0],x[1]+abs(minValue)))
                ...
                print("---Calculate value of precision per Attribute's value: --> Value, Mean, Precision, Standard Deviation")
                #print(precisionRdd.take(5))
                    
                if reShift:
                    print("---Shifting all the values again to replace the correct mean")
                    precisionRdd = precisionRdd.map(lambda (key,mean,prec,dev): (key,mean-abs(minValue),prec,dev))
                    
                if valuePrec:   
                    save_json(precisionRdd,["Value","Mean","Precision","StandardDeviation"],resultFolder + "/precision_values/attribute_" + stringAttribute + "_number_" + str(desiredColumns[j]))
        
                finalPrecisionRdd = finalPrecisionRdd.union(sc.parallelize([(stringAttribute,precisionRdd.map(lambda x: x[1]).mean(),precisionRdd.map(lambda x: x[2]).mean(),precisionRdd.map(lambda x: x[3]).mean(),performanceSample)]))
        ...
        if globalPrec:
            ...
            if reShift:
                attributePrecision = max(0.0, 1.0 - devAttribute/(abs(meanAttribute)+abs(minValue)))
            else:
                attributePrecision = max(0.0, 1.0 - devAttribute/abs(meanAttribute))
        ...
        if attributePrec:    
            ...
            #save file into hdfs
            save_json(finalPrecisionRdd,["Attribute","Mean","Precision","Standard_Deviation","Confidence"],resultFolder + "/precision_attributes_" + str(desiredColumns[j]))
...
def completeness_missing(sc,sqlContext,document,columns,dataTypes,volatiliy,desiredColumns,resultFolder,dimensionColumn,columnsKey,meanAccuracyValues,devAccuracyValues,timelinessAnalyzer,completenessAnalyzer,distinctnessAnalyzer,populationAnalyzer,inputSource,associationRules,totVolume,granularityDimensions,performanceSample):
    ...
    if attributeMiss or valueMiss:
        ...
        for stringIndex in columnsKey:
            ...
            stringSplitIndex = stringIndex.split(",")
            newIndexes = [desiredColumns.index(k) for k in stringSplitIndex]
            newDocument = document.map(lambda x: restriction_filter(x,newIndexes))
            stringAttribute = '_'.join(stringSplitIndex)
		...
		if not itIsTime:
                    ...
                    keyValueDocument = document.map(lambda x: mapping_completeness_missing(x,newIndexes))
                    ...
                    keyValueDocument = keyValueDocument.filter(lambda x: x[0]!="null").filter(lambda x: x[0] is not None).filter(lambda x: x[0]!="").filter(lambda x: x[0]!= "nan").reduceByKey(lambda x,y: (x[0] + y[0],x[1] + y[1]))
        
                    keyValueDocument = keyValueDocument.map(lambda x: (x[0], x[1][1] - x[1][0], x[1][0]/float(x[1][1])))
                    print("---Calculate Completeness Missing for each Attribute's Value: -> ( Value, Missing Values, Completeness Missing Value )")
                    #print(keyValueDocument.take(5))
                
                    save_json(keyValueDocument,["Value","MissingValues","CompletenessMissingValue"],resultFolder + "/completeness_missing_values/" +  stringAttribute)
            ...
            if attributeMiss:
                ...
                #Save both values
                finalCompleteRdd = finalCompleteRdd.union(sc.parallelize([(stringAttribute,totElements - filteredElements,completenessAttribute,performanceSample)]))
    ...
    if attributeMiss:
        ...
        #save file into hdfs
        save_json(finalCompleteRdd,["Attribute","MissingValues","CompletenessMissingValue","Confidence"],resultFolder + "/completeness_missing_attributes")
...
def mapping_completeness_frequency(x,volatiliyTime,performanceSample):
    ...
    try:
        newLine = (x[0], x[3], min(1.0,float(x[3])/(volatiliyTime*(x[4]-x[5])*performanceSample*x[1])))
    except Exception:
        #this exception is raised whenever there is only 1 element available for the current key element ( max = min ), or even if the frequency or the volatiliy is 0, so it will be marked to be removed. (if volatiliy is exceeded there is an error)
        newLine = (x[0], x[3], 2)
...
def completeness_frequency(sc,sqlContext,keyValueCount,volatiliyTime,column,timestamp,resultFolder,valueFreq,performanceSample):
    ...
    #remap with key, count / volatiliyTime * ( max - min ) * update_rate( column, key, initialTime_of_records_in_key, finalTime_of_records_in_key )
    
    completenessFreqDoc = keyValueCount.map(lambda x: mapping_completeness_frequency(x,volatiliyTime,performanceSample)).filter(lambda x: x[2]!=2)
    ...
    if valueFreq:
        save_json(completenessFreqDoc,["Value","RecordNumber","CompletenessFrequencyValue"],resultFolder + "/completeness_frequency_values/" + str(column) + "_" + str(timestamp),None)
...
Args:
	...
	performanceSample(float): Number from 0+ to 1 representing the portion of data that has been considered in the analysis
""" 
def timeliness(sc,sqlContext,document,columns,dataTypes,volatiliy,desiredColumns,resultFolder,dimensionColumn,columnsKey,meanAccuracyValues,devAccuracyValues,timelinessAnalyzer,completenessAnalyzer,distinctnessAnalyzer,populationAnalyzer,inputSource,associationRules,totVolume,granularityDimensions,performanceSample):
    ...
    if completenessAnalyzer is True:
        #get the desired degrees of granularity
        try:
            granularity = granularityDimensions["Completeness_Frequency"].split(",")
    ...
    for j in columns:
        ...
        #load the update rate, if the file is not available an error is returned and the analysis should stop
        if completenessAnalyzer:
            updateValues = False
            try:
                updateRateValuesDF = sqlContext.read.json(inputSource + "/update_rate_values_" + str(desiredColumns[j]))
            ...
            updateGlobal = False
            try:
                updateRateGlobalDF = sqlContext.read.json(inputSource + "/update_rate_global_" + str(desiredColumns[j]) )
        ...
        if updateGlobal & completenessAnalyzer & globalFreq:
            ...
            save_json(completenessHour,["Hour","RecordNumber","CompletenessFrequency"],resultFolder + "/completeness_frequency_global_hour/" + str(desiredColumns[j]))
        ...
        for stringIndex in columnsKey:
            ...
            if updateValues & completenessAnalyzer & (attributeFreq or valueFreq): 
                ...
                #convert to datetime, create the key (value,hour) with the difference in seconds, join the update rate with the same key (value,hour) to obtain the frequency
                ...
                if not newTimeDocument.isEmpty():
                    print("----Select the elements of the update rate with the same key column and join the previous data with it")
                    #print(newTimeDocument.take(5))
                    
                    #reduce the key together, this allow to derive min and max time plus the count of record per key
                    
                    newTimeDocument = newTimeDocument.combineByKey( lambda x: (x[0],1.0,x[0],x[0],x[1]),
                                         lambda x, value: (x[0] + value[0], x[1] + 1, max(x[2],value[0]) , min(x[3],value[0]), max(x[4],value[1])),
                    ...
                    keyValueCount = ( newTimeDocument.map(lambda x: (x[0][0],(x[1][4],x[1][0],x[1][1],x[1][2],x[1][3])))
                                                    .combineByKey( lambda x: (x[0],1.0,x[1],x[2],x[3],x[4]),
                                                                    lambda x, value: (x[0] + value[0], x[1] + 1, x[2] + value[1], x[3] + value[2], max(x[4],value[3]) , min(x[5],value[4])),
                    ...
                    #derive also the completeness frequency using the previous results
                    completenessSumRecord,completenessMeanValue = completeness_frequency(sc,sqlContext,keyValueCount,volatiliyTime,stringAttribute,desiredColumns[j],resultFolder,valueFreq,performanceSample)
                    
                    
                    endTime = time.time()
                        
                    print("-----Completeness Frequency Elapsed Time: " + str(endTime - startTime) +" seconds")
                    
                    if attributeFreq:
                        #completeness frequency rdd
                        finalCompletenessRdd = finalCompletenessRdd.union(sc.parallelize([(stringAttribute,completenessSumRecord,completenessMeanValue,performanceSample)]))
                    ...
                    if timelinessAnalyzer & (attributeTime or valueTime):
                        averageByKey = ( keyValueDocument.map(lambda x: (x[0][0], x[1]))
                                            .combineByKey( lambda x: (x,1.0,x,x),
                                                            lambda x, value: (x[0] + value, x[1] + 1, max(x[2],value) , min(x[3],value)),
		...
		if timelinessMinValue <0:
		    timelinessMinValue = 0.0
                ...
                finalTimeRdd = finalTimeRdd.union(sc.parallelize([(stringAttribute,timelinessMeanValue,timelinessMaxValue,timelinessMinValue,performanceSample,datetimeNow.strftime(stringFormat))]))
            
            if timelinessAnalyzer & valueTime:       
		averageByKey = averageByKey.map(lambda x: (x[0],max(0.0,x[1]),max(0.0,x[2]),max(0.0,x[3]),x[4]))
                save_json(averageByKey,["Value","TimelinessMean","TimelinessMax","TimelinessMin","AnalysisTime"],resultFolder + "/timeliness_values/" + stringAttribute + "_" + str(desiredColumns[j]) )
        ...
        if timelinessAnalyzer & attributeTime:
            ...
            #save file into hdfs
            save_json(finalTimeRdd,["Attribute","TimelinessMean","TimelinessMax","TimelinessMin","Confidence","AnalysisTime"],resultFolder + "/timeliness_attributes_" + str(desiredColumns[j]))
        
        if updateValues & attributeFreq & completenessAnalyzer & ( not finalCompletenessRdd.isEmpty()):
            print("-Value of Completeness per Attribute's value: --> Value, Mean Completeness")
            #print(finalCompletenessRdd.take(5))
            
            #save file into hdfs
            save_json(finalCompletenessRdd,["Attribute","RecordNumber","CompletenessFrequencyValue","Confidence"],resultFolder + "/completeness_frequency_attributes_" + str(desiredColumns[j]))
	    ...
	    attrTimeFreq.append(max(0.0,globalTimeDocument.mean()))
            ...
            attrName.append("Last_Analysis_Timestamp_" + str(desiredColumns[j]))    
            confidence.append(performanceSample)
            
    return attrName,attrTimeFreq,confidence
...
def volume(sc,sqlContext,document,columns,dataTypes,volatiliy,desiredColumns,resultFolder,dimensionColumn,columnsKey,meanAccuracyValues,devAccuracyValues,timelinessAnalyzer,completenessAnalyzer,distinctnessAnalyzer,populationAnalyzer,inputSource,associationRules,totVolume,granularityDimensions,performanceSample):
    ...
    if globalVol:
        ...
        #calculate volume as the fraction between the number of rows of the file after the filters and the requirements and the total available rows before the filtering
    ...
    if valueVol:
        for stringIndex in columnsKey:
            ...
            keyValueDocument = newDocument.map(lambda x: (x,1)).reduceByKey(lambda x,y: x+y).map(lambda x: (x[0],x[1],x[1]/float(totVolume)))
            
            print("--Return the count of rows per Attribute's Value: -> ( Attribute's Value, Count, VolumeValue )")
            #print(keyValueDocument.take(4))
            
            save_json(keyValueDocument,["Value","Count","VolumeValue"],resultFolder + "/volume_values/" +  stringAttribute)
...
def distinctness(sc,sqlContext,document,columns,dataTypes,volatiliy,desiredColumns,resultFolder,dimensionColumn,columnsKey,meanAccuracyValues,devAccuracyValues,timelinessAnalyzer,completenessAnalyzer,distinctnessAnalyzer,populationAnalyzer,inputSource,associationRules,totVolume,granularityDimensions,performanceSample):
    ...
    if populationAnalyzer:
        #get the desired degrees of granularity
        try:
            granularity = granularityDimensions["Completeness_Population"].split(",")
	...
	print(document.take(100))
        distinctDocument = document.map(lambda x: (tuple(x),1)).reduceByKey(lambda x,y: x+y).map(lambda x: float(x[1])/2).filter(lambda x: x>1)
	...
	qualityDistinctness = max(0.0, 1 - (duplicateDocumentLines / float(rowCount)))
    ...
    if attributePop or attributeDistinct:
        previousSource = sqlContext.read.json(inputSource + "/source_distinctness")
        ...
        for stringIndex in columnsKey:
            ...
            if attributePop & populationAnalyzer & (len(stringSplitIndex)==1):
                ...
                previousCompletePopulation = previousSource.filter( previousSource.Attribute == stringSplitIndex[0]).head()["Count"]
                
                qualityCompletenessPopulation = min(1.0,currentCount/float(previousCompletePopulation))
                ...
                finalPopulationRdd = finalPopulationRdd.union(sc.parallelize([(stringSplitIndex[0],currentCount,qualityCompletenessPopulation,performanceSample)]))
            ...
            if attributeDistinct & distinctnessAnalyzer:
                ...
                finalDistinctRdd = finalDistinctRdd.union(sc.parallelize([(stringAttribute,currentCount,qualityDistinctness,performanceSample)]))
        ...
        if attributePop & populationAnalyzer & correctAttributePop:    
            ...
            save_json(finalPopulationRdd,["Attribute","DistinctCount","CompletenessPopulationValue","Confidence"],resultFolder + "/completeness_population_attributes")
        ...
        if attributeDistinct & distinctnessAnalyzer:
            ...
            save_json(finalDistinctRdd,["Attribute","DistinctCount","Distinctness","Confidence"],resultFolder + "/distinctness_attributes")
...
def consistency(sc,sqlContext,document,columns,dataTypes,volatiliy,desiredColumns,resultFolder,dimensionColumn,columnsKey,meanAccuracyValues,devAccuracyValues,timelinessAnalyzer,completenessAnalyzer,distinctnessAnalyzer,populationAnalyzer,inputSource,associationRules,totVolume,granularityDimensions,performanceSample):
    ...
    try:
	    ...
	    print(isRules)
            if not isRules:
	    	rules = sqlContext.createDataFrame([(newCustomRule[0],newCustomRule[1])],["Antecedent","Consequent"])
		isRules = True
	    else:
		newRow = sqlContext.createDataFrame([(newCustomRule[0],newCustomRule[1])],rules.columns)
        ...
        if attributeCon or valueCon:
            ...
            for row in rules.rdd.collect():
                ...
                if valueCon:
                    save_json(consistentRdd,["AntecedentValue","Rule","ConsistencyValue"],resultFolder + "/consistency_values/" + str(row[0]) + "_" + str(row[1]) )
                
                if attributeCon:
                    consistentValue = consistentRdd.map(lambda x: x[2]).mean()
                    print("--Mean Consistent Value for current rule = " + str(consistentValue))
                    
                    partialConsistentRdd = sc.parallelize([(row[0].encode("UTF8","ignore"),row[1].encode("UTF8","ignore"),consistentValue,performanceSample)])
                    
                    finalConsistentRdd = finalConsistentRdd.union(partialConsistentRdd)
        ...
        if attributeCon:
            ...
            meanConsistencyValue = finalConsistentRdd.map(lambda x:x[2]).mean()
            
            #save file into hdfs
            save_json(finalConsistentRdd,["RuleAntecedent","RuleConsequent","ConsistencyValue","Confidence"],resultFolder + "/consistency_attributes_mean_per_rule")
...
def main(sc,sqlContext,configuration_path,performanceSample):
    ...
    #Extract Input Files
    inputSource = configuration_file.filter(lambda x: x[1]==0).map(lambda x: x[0]).collect()[0].encode("UTF8","ignore")
    print(inputSource)
    
    #Extract Input Files
    resultFolder = configuration_file.filter(lambda x: x[1]==1).map(lambda x: x[0]).collect()[0].encode("UTF8","ignore")
    resultFolder = resultFolder + "_confidence_" + str(performanceSample)
    print(resultFolder)

    #Extract Source Quality Requirement
    sourceDesired = int(configuration_file.filter(lambda x: x[1]==12).map(lambda x: x[0]).collect()[0])
    print(sourceDesired)
    ...
    desiredColumns = configuration_file.filter(lambda x: x[1]==2).map(lambda x: x[0]).collect()[0].split(";")
    desiredColumns = [x.encode("UTF8","ignore") for x in desiredColumns]
    print(desiredColumns)
    
    #Extract Key Columns
    keyColumns = configuration_file.filter(lambda x: x[1]==3).map(lambda x: x[0]).collect()[0].split(";")
    keyColumns = [x.encode("UTF8","ignore") for x in keyColumns]
    print(keyColumns)
    
    #Extract Desired Intervals
    desiredIntervalsDim = configuration_file.filter(lambda x: x[1]==4).map(lambda x: x[0]).collect()[0].split(";")
    desiredIntervalsDim = [x.encode("UTF8","ignore") for x in desiredIntervalsDim]
    print(desiredIntervalsDim)
    
    #Extract Desired Elements
    desiredElementsDim = configuration_file.filter(lambda x: x[1]==5).map(lambda x: x[0]).collect()[0].split(";")
    desiredElementsDim = [x.encode("UTF8","ignore") for x in desiredElementsDim]
    print(desiredElementsDim)
    
    #Extract Desired Dimensions
    desiredDimensions = configuration_file.filter(lambda x: x[1]==6).map(lambda x: x[0]).collect()[0].split(";")
    desiredDimensions = [x.encode("UTF8","ignore") for x in desiredDimensions]
    print(desiredDimensions)
    
    #Extract Validity for Timeliness
    volatilities = configuration_file.filter(lambda x: x[1]==7).map(lambda x: x[0]).collect()[0].split(";")
    volatilities = [x.encode("UTF8","ignore") for x in volatilities]
    print(volatilities)
    
    #Extract mean Value for Accuracy
    meanAccuracyValues = configuration_file.filter(lambda x: x[1]==8).map(lambda x: x[0]).collect()[0].split(";")
    meanAccuracyValues = [x.encode("UTF8","ignore") for x in meanAccuracyValues]
    print(meanAccuracyValues)
    
    #Extract maximum deviation for Accuracy
    devAccuracyValues = configuration_file.filter(lambda x: x[1]==9).map(lambda x: x[0]).collect()[0].split(";")
    ...
    associationRules = configuration_file.filter(lambda x: x[1]==10).map(lambda x: x[0]).collect()[0].split(";")
    ...
    try:
        preliminaryInformation = sqlContext.read.json(inputSource + "/preliminary_information").head()
    except Exception:
    ...
    try:
	analysedNames = sqlContext.read.json(inputSource + "/source_analysed_updates")
	analysedName = analysedNames.filter(analysedNames.correctAnalysis == 1).rdd.map(lambda x: (x.fileName)).collect()
	print("ok")
	inputPaths = [(inputSource + "/updates/" + fName + ".txt").encode("UTF8","ignore") for fName in analysedName]
	...
	#InputPaths = ""
    if jparsing:
        data = sc.textFile(inputPaths)
        documentDF = sqlContext.read.option("charset", "UTF8").option("PrimitiveAsString","True").json(data)
    ...
    dimensionColumn = dict((k, dimensionColumn.pop(k, None)) for k in desiredDimensions)
    print(dimensionColumn)
    
    for k in dimensionColumn.iterkeys():
        print(k)
        newList = [desiredNum.index(i) for i in dimensionColumn[k] if i in desiredNum]
    ...
    if not jparsing:
        documentDF = document.toDF(header).select(desiredColumns)
    else:
        documentDF = documentDF.select(desiredColumns)
    ...
    try:
        ...
        Intervals Selection
        """
        newDocumentDF = sqlContext.createDataFrame(sc.emptyRDD(),documentDF.schema)
        
        dictDesiredPosition = dict([(v, k) for k, v in enumerate(desiredColumns)])
        for colo in desiredColumns:
            #newDocumentDF.show(5)
            print(colo)
            
            selectedIntervals = desiredIntervalsDim[dictDesiredPosition[colo]].split(",") 
            if selectedIntervals[0]!="":
		print("Deleting Null")
                documentDF = documentDF.dropna(how="any",subset=[colo])
		...
		extremes = interval.split(":")
                ...
                if (extremes[0] == '') & (len(extremes)==1):
                    ...
                    if colo in dictHeaderNames["Timeliness"]:
                        ...
                        timeFunc =  udf(lambda x: datetime.datetime.strptime(x,stringFormat), TimestampType())
                        
                        #ocumentDF = documentDF.withColumn("Time"+str(colo), unix_timestamp(colo, "dd/MM/yyyy hh:mm:ss.ffffff").cast("double").cast("timestamp"))
    			documentDF = documentDF.withColumn("Time"+str(colo), timeFunc(col(colo)))
                        
                        if (extremes[0] != '') & (extremes[1] != ''):
                            print("both")
                            
                            leftTimestamp = datetime.datetime.strptime(extremes[0],convertFormat)
                            rightTimestamp = datetime.datetime.strptime(extremes[1],convertFormat)
                            #print(documentDF.show(5))
                            
                            splitDocumentDF = documentDF.filter( (col("Time"+str(colo)) >= leftTimestamp) & (col("Time"+str(colo)) <= rightTimestamp) )
                        else:
                            if extremes[0] != '':
                                print("left")
				...
				leftTimestamp = datetime.datetime.strptime(extremes[0],convertFormat)
                                ...
                                splitDocumentDF = documentDF.filter( col("Time"+str(colo)) >= leftTimestamp )
				
                            else:    
                                print("right")
				rightTimestamp = datetime.datetime.strptime(extremes[1],convertFormat)
                                splitDocumentDF = documentDF.filter( col("Time"+str(colo)) <= rightTimestamp )
                        
                        splitDocumentDF = splitDocumentDF.drop("Time"+str(colo))

                    else:
                        if (extremes[0] != '') & (extremes[1] != ''):
                            print("both")
                            splitDocumentDF = documentDF.filter( (col(colo).cast("string") >= str(extremes[0])) & (col(colo).cast("string") <= str(extremes[1])) )
                        else:
                            if extremes[0] != '':
                                print("left")
                                #documentDF = documentDF.filter("" + str(colo) + ">=" + str(extremes[0]))
                                splitDocumentDF = documentDF.filter(col(colo).cast("string") >= str(extremes[0]))
                            else:    
                                print("right")
                                splitDocumentDF = documentDF.filter(col(colo).cast("string") <= str(extremes[1]))
                                
                    #splitDocumentDF.show()
                    print("end interval")
        ...
        """
        for colo in desiredColumns:
            colValues = desiredElementsDim[dictDesiredPosition[colo]].split(",")
		...
		documentDF = documentDF.dropna(how="any",subset=[colo])
                ...
                documentDF = documentDF.filter(col(colo).isin(colValues))
                #documentDF.show()
    ...
    if "Completeness_Frequency" in dimensionColumn.keys():
        ...
        if "Timeliness" in dimensionColumn:
            ...
            timelinessAnalyzer = False
            dimensionColumn["Timeliness"] = dimensionColumn["Completeness_Frequency"]
    ...
    if "Completeness_Population" in dimensionColumn.keys():
        ...
        if "Distinctness" in dimensionColumn:
            ...
            distinctnessAnalyzer = False
            dimensionColumn["Distinctness"] = dimensionColumn["Completeness_Population"]
    ...
    if performanceSample < 1:
        documentDF = documentDF.sample(False,performanceSample)
    ...
    if "Completeness_Missing" in dimensionColumn.keys():
	    ...
	    start = time.time()
            ...
            qualityName,partialQuality,confidence = completeness_missing(sc,sqlContext,document,dimensionColumn["Completeness_Missing"],dataTypes,volatilities,desiredColumns,resultFolder,dimensionColumn,keyColumns,meanAccuracyValues,devAccuracyValues,timelinessAnalyzer,completenessAnalyzer,distinctnessAnalyzer,populationAnalyzer,inputSource,associationRules,totVolume,granularityDimensions,performanceSample)
    ...
    dimensionName = {'Accuracy':accuracy, 'Precision':precision,'Completeness_Missing':completeness_missing,'Distinctness':distinctness,'Consistency':consistency,'Timeliness':timeliness,'Volume':volume}
    ...
    for i in dimensionColumn.iterkeys():
        ...
        try:
            ...
            qualityName,partialQuality,confidence = func(sc,sqlContext,document,dimensionColumn[i],dataTypes,volatilities,desiredColumns,resultFolder,dimensionColumn,keyColumns,meanAccuracyValues,devAccuracyValues,timelinessAnalyzer,completenessAnalyzer,distinctnessAnalyzer,populationAnalyzer,inputSource,associationRules,totVolume,granularityDimensions,performanceSample)
            ...
            if i =="Timeliness":
                if timelinessAnalyzer & completenessAnalyzer:
                    print("- " + str(i) + " + Completeness_Frequency " + " Elapsed Time: " + str(end - start) + " seconds")
                    timeRdd = timeRdd.union(sc.parallelize([(str(i) + "Completeness_Frequency",(end - start))]))
                else:
                    if completenessAnalyzer:
                        print("Completeness_Frequency " + " Elapsed Time: " + str(end - start) + " seconds")
                        timeRdd = timeRdd.union(sc.parallelize([("Completeness_Frequency",(end - start))]))
                    else:
            ...
            if i =="Distinctness":
                if distinctnessAnalyzer & populationAnalyzer:
                    print("- " + str(i) + " + Completeness_Population " + " Elapsed Time: " + str(end - start) + " seconds")
                    timeRdd = timeRdd.union(sc.parallelize([(str(i) + "Completeness_Population",(end - start))]))
                else:
                    if populationAnalyzer:
                        print("Completeness_Population " + " Elapsed Time: " + str(end - start) + " seconds")
                        timeRdd = timeRdd.union(sc.parallelize([("Completeness_Population",(end - start))]))
                    else:
                        print(str(i) + " Elapsed Time: " + str(end - start) + " seconds")
                        timeRdd = timeRdd.union(sc.parallelize([(str(i),(end - start))])) 
...
if __name__ == "__main__":
    ...
    correctAnalysis = main(sc,sqlContext,configuration_path,performanceSample)
